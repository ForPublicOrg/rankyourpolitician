...
export function CountRow({
    ...
    <>
      <div className="flex min-w-0 items-baseline justify-between gap-3">
        <div className="min-w-0">
          <span className={clsx('truncate text-sm', isLeader ? 'font-extrabold text-ink' : 'font-semibold text-ink-soft')}>
            {row.isNota ? tr('elections.nota') : row.name}
          </span>
          {!row.isNota && <span className="ml-2 text-xs text-ink-faint">{row.party}</span>}
        </div>
  ...
  if (row.isNota || !row.candidateSlug) {
    return <li className="rounded-2xl border border-line bg-paper-soft p-3">{body}</li>;
  }
  return (
    <li>
      <Link
        href={`/elections/${seatSlug}/${row.candidateSlug}`}
        className="pressable block rounded-2xl border border-line bg-paper-soft p-3 transition hover:border-brand/40 hover:shadow-lift"
      >
        {body}
      </Link>
    </li>
